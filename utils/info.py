...
def information(args):
    ...
    if math.isclose(args.malicious, 0) == False:
        ...
        info.append(f'    Malicious per round: {getattr(args, "malicious_per_round", "n/a")}')
    ...
    info.append(f'    frac (sampling ratio): {args.frac}')
    info.append(f'    Local batch size: {args.local_bs}')
    info.append(f'    Local epoch: {args.local_ep}')
    info.append(f'    Client_LR: {args.lr}')
    info.append(f'    CIFAR scale: {getattr(args, "cifar_scale", "n/a")}')
    info.append(f'    CNN scale: {getattr(args, "cnn_scale", "n/a")}')
    ...
    elif args.defence == 'krum' or args.defence == 'multikrum':
        if args.turn != 0 and args.malicious != 0:
...
def get_base_info(args):
    if args.defence == 'RLR':
         base_info = '{}_{}_{}_{}_{}_{}_{}'.format(args.dataset,
                args.model, args.defence, args.robustLR_threshold,args.layer_threshold ,args.global_threshold,int(time.time()))
    else:
        base_info = '{}_{}_{}_{}_{}_{}'.format(args.dataset,
                    args.model, args.defence, args.layer_threshold ,args.global_threshold,int(time.time()))
    if math.isclose(args.malicious, 0) == False:
