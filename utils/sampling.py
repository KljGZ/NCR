...
def non_iid_distribution_client(group_proportion, num_clients, num_classes):
    ...
    for i in range(num_classes):
        ...
        for j in range(num_each_group):
            ...
            # use a flat incremental id to avoid assuming 10 classes
...
def dirichlet_split(labels, num_clients, alpha=0.5, min_size=10, seed=None):
    """
    Generic Dirichlet label-skew splitter (supports any num_classes).
    - labels: list/array of class labels
    - alpha: smaller => more skew
    - min_size: enforce minimum samples per client
    Returns dict: client_id -> set(indices)
    """
    ...
    while min_shard < min_size:
        ...
        min_shard = min(len(x) for x in idx_batch)
    for i in range(num_clients):
        dict_users[i] = set(idx_batch[i])
    return dict_users
