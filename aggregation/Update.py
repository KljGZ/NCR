...
class LocalUpdate(object):
    ...
    def train(self, net):
        ...
        net.train()
        # train and update
        base_lr = getattr(self.args, "client_lr", self.args.lr)
        lr_decay = getattr(self.args, "lr_decay", 1.0)
        round_idx = getattr(self.args, "current_round", 0)
        lr = base_lr * (lr_decay ** round_idx)
        weight_decay = getattr(self.args, "wd", 0.0)
        optimizer = torch.optim.SGD(
            net.parameters(), lr=lr, momentum=self.args.momentum, weight_decay=weight_decay)
        scheduler = ExponentialLR(optimizer, gamma=lr_decay) if lr_decay < 1.0 else None
        ...
        fisher = self.compute_fisher(net)
        return net.state_dict(), sum(epoch_loss) / len(epoch_loss), fisher
