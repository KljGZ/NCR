...
def detection(score, nobyz):
    ...
    n_clients = int(score.shape[0])
    nobyz = int(min(max(nobyz, 0), n_clients))
    real_label = np.ones(n_clients)
    real_label[:nobyz] = 0
    # metrics are optional diagnostics; guard empty slices
    acc = len(label_pred[label_pred == real_label]) / max(n_clients, 1)
    recall = 1 - (np.sum(label_pred[:nobyz]) / max(nobyz, 1))
    fpr = 1 - (np.sum(label_pred[nobyz:]) / max(n_clients - nobyz, 1))
    fnr = np.sum(label_pred[:nobyz]) / max(nobyz, 1)
...
def detection1(score):
    ...
    denom = (max_v - min_v)
    if denom <= 1e-12:
        # all scores are (almost) identical -> cannot cluster meaningfully -> treat as no attack
        print('No attack detected! (degenerate scores)')
        return 0
    score = (score - min_v) / denom
